...
abstract class BattleTypedSearch<T extends SearchType> {
	...
	'dlc1' | 'dlc1doubles' | 'stadium' | 'sadex' | null = null;
	...
	constructor(searchType: T, format = '' as ID, speciesOrSet: ID | PokemonSet = '' as ID) {
		...
		if (format.includes('sigmadex') || format.startsWith('sa') || format.includes('sadex')) {
			format = (format.startsWith('sa') ? format.slice(2) :
				format.includes('sadex') ? format.slice(6) : format.slice(11)) as ID;
	...
	getTier(pokemon: Species) {
		...
		const tableKey = this.formatType === 'doubles' ? `gen${gen}doubles` :
			...
			this.formatType === 'sadex' ? `gen${gen}sadex` :
			`gen${gen}`;
...
class BattlePokemonSearch extends BattleTypedSearch<'pokemon'> {
	...
	getBaseResults(): SearchRow[] {
		...
		else if (format === 'lcubers') tierSet = tierSet.slice(slices["LC Ubers"] || slices.LC || slices.ZU)
		...
		if (sketch || isHackmons) {
			...
			for (let id in BattleMovedex) {
				...
				if (sketch) {
					...
					if (move.isNonstandard === 'Past' && this.formatType !== 'sadex') continue;
