...
var _template49=this.template(_templateId4,_effect11,'NODEFAULT');
if(!_template49){
if(_line15)return _line15;
_template49=this.template('activate');
return _line15+_template49.replace('[EFFECT]',this.effect(_effect11));
}

if(_id14==='brickbreak'){
_template49=_template49.replace('[TEAM]',this.team(_target2.slice(0,2)));
}
if(kwArgs.ability){
...
_template49=_template49.replace('[MOVE]',kwArgs.move).replace('[NUMBER]',kwArgs.number).replace('[ITEM]',kwArgs.item).replace('[NAME]',kwArgs.name);
}
return _line15+_template49.replace('[POKEMON]',this.pokemon(_pokemon19)).replace('[TARGET]',this.pokemon(_target2)).replace('[SOURCE]',this.pokemon(kwArgs.of));
}
...
return _line18+_template53.replace('[POKEMON]',this.pokemon(_pokemon23)).replace('[STAT]',BattleTextParser.stat(_stat2)).replace('[ITEM]',this.effect(kwArgs.from));
}
