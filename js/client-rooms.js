	...
	this.RoomsRoom = Room.extend({
		...
		updateRoomList: function () {
			...
			if (rooms.userCount) {
				...
				var leftSide = '<button class="button" name="finduser" title="Find an online user"><span class="pixelated usercount" title="252+ SpA Choice Specs Adaptability Tera Normal Porygon-Z Hyper Beam vs. 252 HP / 252+ SpD Blissey on a critical hit: 661-778 (92.5 - 108.9%) -- 56.3% chance to OHKO. #TeamPorygon-Z" ></span><strong>' + userCount + '</strong> ' + (userCount == 1 ? 'user' : 'users') + ' online</button> ';
				var rightSide = '<button class="button" name="roomlist" title="Watch an active battle"><span class="pixelated battlecount" title="A T-Tar shivers when a Leavanny hits an Axe Kick. #TeamLeavanny" ></span><strong>' + battleCount + '</strong> active ' + (battleCount == 1 ? 'battle' : 'battles') + '</button>';
