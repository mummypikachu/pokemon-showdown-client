...
var _id4=BattleTextParser.effectId(_effect3);
if(_id4==='typechange'){
...
var _template12=this.template('start',_id4.slice(0,_id4.length-3));
...
if(_id4==='mist'&&this.gen<=2)templateId='startGen'+this.gen;
if(_id4==='reflect'||_id4==='lightscreen')templateId='startGen1';
...
var _target='';
if(['magician','pickpocket'].includes(_id7)){var _ref2=
...
var _id9=BattleTextParser.effectId(_effect5);
if(_id9==='instruct'){
...
var _id0=BattleTextParser.effectId(_effect1);
if(_id0==='celebrate'){
return this.template('activate','celebrate').replace('[TRAINER]',this.trainer(_pokemon17.slice(0,2)));
}
if(!_target2&&
['hyperdrill','hyperspacefury','hyperspacehole','phantomforce','shadowforce','feint'].includes(_id0)){var _ref3=
...
if(_id0==='lockon'||_id0==='mindreader'){
...
if((_id0==='mummy'||_id0==='lingeringaroma')&&kwArgs.ability){
_line13+=this.ability(kwArgs.ability,_target2);
_line13+=this.ability(_id0==='mummy'?'Mummy':'Lingering Aroma',_target2);
var _template46=this.template('changeAbility',_id0);
return _line13+_template46.replace('[TARGET]',this.pokemon(_target2));
}

var _templateId4='activate';
if(_id0==='forewarn'&&_pokemon17===_target2){
_templateId4='activateNoTarget';
}
if((_id0==='protosynthesis'||_id0==='quarkdrive')&&kwArgs.fromitem){
_templateId4='activateFromItem';
}
if(_id0==='orichalcumpulse'&&kwArgs.source){
_templateId4='start';
}
var _template47=this.template(_templateId4,_effect1,'NODEFAULT');
if(!_template47){
if(_line13)return _line13;
_template47=this.template('activate');
return _line13+_template47.replace('[EFFECT]',this.effect(_effect1));
}
...
return _line13+_template47.replace('[POKEMON]',this.pokemon(_pokemon17)).replace('[TARGET]',this.pokemon(_target2)).replace('[SOURCE]',this.pokemon(kwArgs.of));
}

case'-prepare':{
var _pokemon18=args[1],_effect10=args[2],_target3=args[3];
var _template48=this.template('prepare',_effect10);
return _template48.replace('[POKEMON]',this.pokemon(_pokemon18)).replace('[TARGET]',this.pokemon(_target3));
}
...
var _id1=BattleTextParser.effectId(kwArgs.from);
if(_template49){
return _line14+_template49.replace('[POKEMON]',this.pokemon(_pokemon19));
}

if(!kwArgs.from){
_template49=this.template(percentage?'damagePercentage':'damage');
return _line14+_template49.replace('[POKEMON]',this.pokemon(_pokemon19)).replace('[PERCENTAGE]',percentage);
}
if(kwArgs.from.startsWith('item:')){
_template49=this.template(kwArgs.of?'damageFromPokemon':'damageFromItem');
return _line14+_template49.replace('[POKEMON]',this.pokemon(_pokemon19)).replace('[ITEM]',this.effect(kwArgs.from)).replace('[SOURCE]',this.pokemon(kwArgs.of));
}
if(kwArgs.partiallytrapped||_id1==='bind'||_id1==='wrap'){
_template49=this.template('damageFromPartialTrapping');
return _line14+_template49.replace('[POKEMON]',this.pokemon(_pokemon19)).replace('[MOVE]',this.effect(kwArgs.from));
}
...
var _line15=this.maybeAbility(kwArgs.from,_pokemon20);
if(_template50){
return _line15+_template50.replace('[POKEMON]',this.pokemon(_pokemon20)).replace('[SOURCE]',this.pokemon(kwArgs.of)).replace('[NICKNAME]',kwArgs.wisher);
}

if(kwArgs.from&&!kwArgs.from.startsWith('ability:')){
_template50=this.template('healFromEffect');
return _line15+_template50.replace('[POKEMON]',this.pokemon(_pokemon20)).replace('[EFFECT]',this.effect(kwArgs.from));
}

_template50=this.template('heal');
return _line15+_template50.replace('[POKEMON]',this.pokemon(_pokemon20));
}
...
var _template51=this.template(_templateId5+'FromItem',kwArgs.from);
return _line16+_template51.replace('[POKEMON]',this.pokemon(_pokemon21)).replace('[STAT]',BattleTextParser.stat(_stat2)).replace('[ITEM]',this.effect(kwArgs.from));
}
var _template52=this.template(_templateId5,kwArgs.from);
return _line16+_template52.replace('[POKEMON]',this.pokemon(_pokemon21)).replace('[STAT]',BattleTextParser.stat(_stat2));
}
...
var _template53=this.template('boost',_effect11);
...
if(_id10==='guardswap')_templateId6='swapDefensiveBoost';
if(_id10==='powerswap')_templateId6='swapOffensiveBoost';
var _template54=this.template(_templateId6,kwArgs.from);
return _line18+_template54.replace('[POKEMON]',this.pokemon(_pokemon23)).replace('[TARGET]',this.pokemon(_target4));
}
...
return _line19+_template55.replace('[POKEMON]',this.pokemon(_pokemon24)).replace('[TARGET]',this.pokemon(_target5));
}
...
return _line20+_template56.replace('[POKEMON]',this.pokemon(_pokemon25)).replace('[SOURCE]',this.pokemon(source));
}
...
return _line21+_template57.replace('[POKEMON]',this.pokemon(_pokemon26));
}
...
return _template58.replace('[POKEMON]',this.pokemon(_pokemon27));
}
...
if(_id11==='mist'&&this.gen<=2)_templateId9='blockGen'+this.gen;
var _template59=this.template(_templateId9,_effect12);
return _line22+_template59.replace('[POKEMON]',this.pokemon(_pokemon28)).replace('[SOURCE]',this.pokemon(attacker||kwArgs.of)).replace('[MOVE]',_move2);
}

case'-fail':{
var _pokemon29=args[1],_effect13=args[2],_stat3=args[3];
var _id12=BattleTextParser.effectId(_effect13);
...
!['sunnyday','raindance','sandstorm','hail','snowscape','chillyreception'].includes(_id12)){
_templateId0='blockMove';
}else if(blocker==='uproar'&&kwArgs.msg){
_templateId0='blockSelf';
}
var _template60=this.template(_templateId0,kwArgs.from);
if(_template60){
return _line23+_template60.replace('[POKEMON]',this.pokemon(_pokemon29));
}

if(_id12==='unboost'){
_template60=this.template(_stat3?'failSingular':'fail','unboost');
return _line23+_template60.replace('[POKEMON]',this.pokemon(_pokemon29)).replace('[STAT]',_stat3);
}

_templateId0='fail';
if(['brn','frz','par','psn','slp','substitute','shedtail'].includes(_id12)){
...
_template60=this.template(_templateId0,_id12);
return _line23+_template60.replace('[POKEMON]',this.pokemon(_pokemon29));
}
...
_template61=this.template(_pokemon30?_templateId1:'immuneNoPokemon',kwArgs.from);
}
return _line24+_template61.replace('[POKEMON]',this.pokemon(_pokemon30));
}
...
return _line25+_template62.replace('[SOURCE]',this.pokemon(_source));
}
...
if(!_id13&&cmd==='-mega'&&this.gen<7)_templateId10='megaGen6';
if(!_item2&&cmd==='-mega')_templateId10='megaNoItem';
var _template64=this.template(_templateId10,_id13);
...
var _template65=this.template(_templateId11,_id14);
var _pokemonName=this.pokemon(_pokemon33);
return _template65.replace('[POKEMON]',_pokemonName).replace('[TYPE]',type);
}

case'-zpower':{
var _pokemon34=args[1];
var _template66=this.template('zPower');
return _template66.replace('[POKEMON]',this.pokemon(_pokemon34));
}

case'-burst':{
var _pokemon35=args[1];
var _template67=this.template('activate',"Ultranecrozium Z");
return _template67.replace('[POKEMON]',this.pokemon(_pokemon35));
}
...
return _template69.replace('[POKEMON]',this.pokemon(_pokemon37)).replace('[TARGET]',this.pokemon(_target6));
}
